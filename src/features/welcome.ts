...
const welcome = (app: App) => {
	...
	app.action('hello_hacker', async ({ ack, body, action }) => {
		...
		const introPieces: (string | [string, number])[] = [
			[`*flaps hand vigorously* Hello to you too (again), <@${user}>!`, 1000],
			[
				`Anyways, small talk over, let's get into the gist of things. I'm here to help you understand what HN is, and some ways that you might be able to use it.`,
				2500,
			],
	...
	app.action('balance_hacker', async ({ ack, body, action }) => {
		...
		const introPieces: (string | [string, number])[] = [
			[`Woot woot! :yay:`, 1000],
			[
				`As I said before, \`/balance\` is a *slash command* that you can run anywhere (except threads) to get your current account balance :classical_building:. If you want to be a bit more discreet about it, you can run \`/peek\`. To see other peoples' balances, run \`/peek <@user>\`: something like \`/peek <@U013STH0TNG>\` (feel free to run it right now!)`,
				2500,
			],
			[
				'To transfer money *out of your account*, there are a bunch of different commands that you can use. The simplest one is /send-hn, which allows you to, well, send HN to someone (or something).',
				1000,
			],
	...
	app.action('advance_hacker', async ({ ack, body, action }) => {
		...
		await sendSequentially(
			[
				...
				[
					'You can find the source code of the HN API at https://z.rishi.cx/g/hn and the source code for yours truly at https://z.rishi.cx/g/teller.',
					1000,
				],
				'.',
				'.',
